...
export function QuickTabDialog({ open, onClose, onCreated, notify }: { open: boolean; onClose: () => void; onCreated: (tab: QuickTab, invitation: InvitationResult) => Promise<void>; notify: (message: string) => void }) {
  ...
  const reset = () => { setItems([emptyItem()]); setReceipt(null); setStatus("Inget kvitto valt ännu."); setStatusClass(""); setValues({ name: "", merchant: "", receiptDate: "", total: "" }); setError(""); };
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = await api<{ quickTab: QuickTab; invitation: InvitationResult }>("/api/quick-tabs", { method: "POST", body: { ...values, items: items.map(({ name, quantity, amount }) => ({ name, quantity, amount })) } });
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Kunde inte skapa snabbnotan"); }
  };
  ...
  return <Modal open={open} onClose={close} wide><form onSubmit={submit}><DialogHeader eyebrow="Separat från resorna" title="Skanna en snabbnota" onClose={close} /><section className="receipt-capture"><div className="receipt-capture-copy"><strong>Fotografera hela kvittot</strong><small>Ta ett nytt foto eller välj en befintlig bild. Bildvalet finns alltid kvar om Chrome-kameran inte startar korrekt.</small></div><div className="receipt-actions"><label className="button dark receipt-camera-button">▣ Ta foto<input type="file" accept="image/*" capture="environment" hidden onChange={(event) => void analyze(event.target.files?.[0])} /></label><label className="button ghost receipt-camera-button">Välj bild<input type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={(event) => void analyze(event.target.files?.[0])} /></label></div><p className={`receipt-status ${statusClass}`} aria-live="polite">{status}</p></section>
    ...
    <div className="quick-item-heading"><div><p className="eyebrow">Kvittorader</p><h3>Kontrollera mat och dryck</h3></div><button className="button ghost small-button" type="button" onClick={() => setItems((current) => [...current, emptyItem()])}>＋ Lägg till rad</button></div><div className="quick-item-editor">{items.map((item) => <div className="quick-edit-row" key={item.key}><label className="grow">Rätt eller dryck<input value={item.name} maxLength={100} required onChange={(event) => setItems((current) => current.map((row) => row.key === item.key ? { ...row, name: event.target.value } : row))} /></label><label>Antal<input type="number" min="1" max="20" step="1" value={item.quantity} required onChange={(event) => setItems((current) => current.map((row) => row.key === item.key ? { ...row, quantity: Number(event.target.value) } : row))} /></label><label>Radsumma SEK<input type="number" min="0.01" step="0.01" inputMode="decimal" value={item.amount} required onChange={(event) => setItems((current) => current.map((row) => row.key === item.key ? { ...row, amount: event.target.value } : row))} /></label><button className="delete-button quick-remove-item" type="button" aria-label="Ta bort rad" onClick={() => setItems((current) => current.length === 1 ? [emptyItem()] : current.filter((row) => row.key !== item.key))}>×</button></div>)}</div>
    <p className="split-summary">{items.length} {items.length === 1 ? "rad" : "rader"} · {formatMoney(rowCents)}{totalCents ? difference === 0 ? " · stämmer med totalen" : ` · ${formatMoney(Math.abs(difference))} ${difference > 0 ? "saknas i raderna" : "över totalen"}` : ""}</p><p className="form-error" role="alert">{error}</p><button className="button primary wide dialog-submit" type="submit">Skapa och bjud in <span>→</span></button></form></Modal>;
}
