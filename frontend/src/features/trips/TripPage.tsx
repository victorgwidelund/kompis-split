...
export function TripPage({ trip, user, categories, onBack, onRefresh, onOpenDialog, notify }: Props) {
  ...
  return <section className="trip-view">
    ...
    {tab === "overview" && <div>{isMobile ? <div className="trip-summary-strip">
      ...
      </div> : <div className="summary-grid"><article className="hero-stat coral"><span className="stat-icon">↗</span><p>Totalt spenderat</p><strong>{formatMoney(trip.totalCents)}</strong><small>{trip.expenses.length === 1 ? "1 utgift registrerad" : `${trip.expenses.length} utgifter registrerade`}</small></article><article className="hero-stat cobalt"><span className="stat-icon">◎</span><p>Kvar att göra upp</p><strong>{formatMoney(openAmount)}</strong><small>{trip.settlements.length ? `${trip.settlements.length} betalningar kvar` : "Alla ligger jämnt"}</small></article><article className="member-stat"><p>Är med i gruppen</p><div className="avatar-stack">{trip.participants.slice(0, 5).map((item, index) => <Avatar name={item.name} index={index} key={item.id} />)}</div><strong>{trip.participants.length} {trip.participants.length === 1 ? "person" : "personer"}</strong>{manager && !archived && <button className="text-button" onClick={() => onOpenDialog("person")}>Lägg till någon →</button>}</article></div>}<div className="dashboard-grid"><section className="panel"><div className="panel-title"><div><p className="eyebrow">Det senaste</p><h2>Senaste utgifterna</h2></div><button className="text-button" onClick={() => setTab("expenses")}>Visa alla →</button></div><div className="expense-list">{trip.expenses.length ? trip.expenses.slice(0, 4).map((expense) => expenseRow(expense, false)) : <EmptyState title="Inget spenderat ännu">Lägg till den första utgiften när någon tar notan.</EmptyState>}</div></section><section className="panel balance-panel"><div className="panel-title"><div><p className="eyebrow">Snabbkoll</p><h2>Saldon</h2></div></div><div className="balance-list">{trip.participants.map((item, index) => { const balance = Number(trip.balances[item.id] || 0); return <button type="button" className="balance-row" key={item.id} onClick={() => setViewingParticipant(item)}><Avatar name={item.name} index={index} /><div className="balance-name"><strong>{item.name}</strong><small>{balance > 0 ? "får tillbaka" : balance < 0 ? "är skyldig" : "ligger jämnt"}</small></div><strong className={balance > 0 ? "positive" : balance < 0 ? "negative" : ""}>{balance ? formatMoney(Math.abs(balance)) : "—"}</strong></button>; })}</div><button className="button dark wide" onClick={() => setTab("settle")}>Se vem som betalar vem <span>→</span></button></section></div></div>}
    {tab === "expenses" && <section className="panel full-panel"><div className="panel-title"><div><p className="eyebrow">Varje öre</p><h2>Alla utgifter</h2></div>{!archived && <button className="button dark" onClick={() => onOpenDialog("expense")}>＋ Lägg till utgift</button>}</div><div className="expense-list roomy">{trip.expenses.length ? trip.expenses.map((expense) => expenseRow(expense)) : <EmptyState title="Inga kvitton, inga problem">Utgifterna visas här när ni lägger till dem.</EmptyState>}</div></section>}
    ...
    {tab === "people" && <section className="panel full-panel"><div className="panel-title"><div><p className="eyebrow">Gänget</p><h2>Personer i gruppen</h2></div>{manager && !archived && <button className="button dark" onClick={() => onOpenDialog("person")}>＋ Lägg till vän</button>}</div><div className="people-grid">{trip.participants.length ? trip.participants.map((item, index) => <button type="button" className="person-card" key={item.id} onClick={() => setViewingParticipant(item)}><Avatar name={item.name} index={index} /><strong>{item.name}</strong><small>{item.userId ? "Registrerad användare" : "Gäst"}</small><small>{item.swishPhone ? `Swish ${item.swishPhone}` : "Inget Swish-nummer"}</small></button>) : <EmptyState title="Vilka ska med?">Lägg till vänner för att börja dela utgifter.</EmptyState>}</div></section>}
