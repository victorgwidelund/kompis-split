...
import type { Category, Expense, Participant, ReminderResult, Settlement, Trip, User } from "../../types/models";
...
export function TripPage({ trip, user, categories, onBack, onRefresh, onOpenDialog, onRemindUnpaid, notify }: Props) {
  ...
  const myOpenAmount = trip.settlements.filter((item) => person(item.toId)?.userId === user.id).reduce((sum, item) => sum + item.amountCents, 0);
  const remindUnpaid = async () => {
    if (!confirm(`Skicka en påminnelse via mail och push till alla som är skyldiga dig pengar i ${trip.name}?`)) return;
    setReminding(true);
    try {
      const result = await onRemindUnpaid(trip.id);
      notify(result.total === 0 ? "Ingen är skyldig dig något i den här gruppen just nu" : `Skickade ${result.sent} av ${result.total} påminnelser${result.errors.length ? ` (${result.errors.length} misslyckades)` : ""}`);
  ...
  const participantPayments = viewingParticipant ? trip.payments.filter((payment) => payment.fromId === viewingParticipant.id) : [];
  const participantPaymentsTotal = participantPayments.reduce((sum, payment) => sum + payment.amountCents, 0);
  return <section className="trip-view">
    ...
    <Modal open={Boolean(viewingParticipant)} onClose={() => setViewingParticipant(null)} wide><DialogHeader eyebrow="Person" title={viewingParticipant?.name || ""} onClose={() => setViewingParticipant(null)} /><p className="eyebrow">Betalade för utgifter</p><p className="muted">{participantExpenses.length ? `Betalade för ${participantExpenses.length} ${participantExpenses.length === 1 ? "utgift" : "utgifter"} · ${formatMoney(participantTotal)} totalt` : "Har inte betalat för någon utgift ännu."}</p><div className="expense-list roomy">{participantExpenses.map((expense) => expenseRow(expense, false))}</div><p className="eyebrow">Har betalat till andra</p><p className="muted">{participantPayments.length ? `Betalade ${participantPayments.length} ${participantPayments.length === 1 ? "gång" : "gånger"} · ${formatMoney(participantPaymentsTotal)} totalt` : "Har inte betalat till någon ännu."}</p><div className="expense-list roomy">{participantPayments.length ? participantPayments.map((payment) => <article className="expense-row" key={payment.id}><span className="category-icon">✓</span><div className="expense-main"><strong>Betalade {person(payment.toId)?.name || "Okänd"}</strong><small>{payment.note || "Betalning"} · {formatDate(payment.paidAt)}</small></div><div className="expense-amount positive">{formatMoney(payment.amountCents)}</div></article>) : <EmptyState title="Inga betalningar ännu">Registrerade betalningar till andra visas här.</EmptyState>}</div></Modal>
  </section>;
}
