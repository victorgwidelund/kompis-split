...
interface Props {
  ...
  onOpenDialog: (dialog: TripDialog, expense?: Expense, payment?: PaymentPreset) => void;
...
export function TripPage({ trip, user, categories, onBack, onRefresh, onOpenDialog, notify }: Props) {
  ...
  return <section className="trip-view">
    ...
    {tab === "people" && <section className="panel full-panel"><div className="panel-title"><div><p className="eyebrow">Gänget</p><h2>Personer i gruppen</h2></div>{manager && !archived && <button className="button dark" onClick={() => onOpenDialog("person")}>＋ Lägg till vän</button>}</div><div className="people-grid">{trip.participants.length ? trip.participants.map((item, index) => <article className="person-card" key={item.id}><Avatar name={item.name} index={index} /><strong>{item.name}</strong><small>{item.userId ? "Registrerad användare" : "Gäst"}</small><small>{item.swishPhone ? `Swish ${item.swishPhone}` : "Inget Swish-nummer"}</small></article>) : <EmptyState title="Vilka ska med?">Lägg till vänner för att börja dela utgifter.</EmptyState>}</div></section>}
  </section>;
}
