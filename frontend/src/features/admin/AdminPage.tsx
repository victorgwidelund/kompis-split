...
interface EmailSettingsPanelProps {
  ...
  onRemind: () => Promise<ReminderResult>;
}

function EmailSettingsPanel({ settings, onSave, onTest, onRemind }: EmailSettingsPanelProps) {
  ...
  const remind = async () => {
    if (!confirm("Skicka betalningspåminnelser till alla som har obetalda belopp?")) return;
    setReminding(true); setRemindStatus("");
    try {
      const result = await onRemind();
      setRemindStatus(`Skickade ${result.sent} av ${result.total} påminnelser.${result.errors.length ? ` Fel: ${result.errors.join("; ")}` : ""}`);
    } catch (error) { setRemindStatus(error instanceof Error ? error.message : "Kunde inte skicka påminnelser"); }
  ...
  return <section className="panel admin-panel"><div className="panel-title"><div><p className="eyebrow">E-post</p><h2>Microsoft Graph</h2></div><span className={`role-badge ${settings.configured ? "positive-badge" : "muted-badge"}`}>{settings.configured ? "Konfigurerad" : "Ej konfigurerad"}</span></div>
    ...
    <div className="email-settings-actions"><span className="grow"><strong>Betalningspåminnelser</strong><small>Skickar ett mail per person med obetalda belopp i grupper och snabbnotor.</small></span><button className="button ghost small-button" type="button" onClick={() => void remind()} disabled={reminding || !settings.configured}>{reminding ? "Skickar…" : "Skicka påminnelser"}</button></div>
    {remindStatus && <small className="email-settings-status">{remindStatus}</small>}
  </section>;
}
...
export function AdminPage({ data, currentUserId, demoMode, onEnterDemo, onBack, onRefresh, onOpenTrip, onUserUpdate, onTripArchive, onTripRestore, onQuickTabDelete, onBugReportResolve, onBugReportDelete, onEmailSettingsSave, onEmailSettingsTest, onRemindUnpaid }: Props) {
  ...
  return <section className="admin-view">
    ...
    <EmailSettingsPanel key={data.emailSettings.updatedAt || "unset"} settings={data.emailSettings} onSave={onEmailSettingsSave} onTest={onEmailSettingsTest} onRemind={onRemindUnpaid} />
