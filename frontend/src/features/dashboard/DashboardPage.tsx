...
export function DashboardPage({ user, dashboard, quickTabs, categories, onNavigate, onNewTrip, onNewQuickTab, onInviteFriend }: Props) {
  ...
  return <section className="home-dashboard">
    <header className="dashboard-heading"><div><p className="eyebrow">Din överblick</p><h1>Hej, {user.name.split(/\s+/)[0]}!</h1><p>Alla resor, utgifter och saldon på ett ställe.</p></div><div className="dashboard-heading-actions"><button className="button ghost" onClick={() => onNavigate({ page: "statistics" })}>⌁ Statistik</button><button className="button primary" onClick={onNewTrip}>＋ Ny resa</button></div></header>
    <div className="summary-grid home-stats">
      <button className="hero-stat coral stat-link" type="button" onClick={scrollToTrips}><span className="stat-icon">✦</span><p>Aktiva resor</p><strong>{active.length}</strong><small>Visa pågående och kommande →</small></button>
