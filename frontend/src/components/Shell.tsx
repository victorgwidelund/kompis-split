  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <button className="button dark wide" onClick={onNewTrip}><span>＋</span> Ny resa</button>
        ...
        <div className="side-heading"><span>Aktiva resor</span><span className="count">{active.length}</span></div>
        <nav className="trip-list" aria-label="Aktiva resor">{active.length ? active.map(tripLink) : <small className="side-empty">Inga aktiva resor</small>}</nav>
        {!!archived.length && <div className="archive-section"><div className="side-heading"><span>Arkiv</span><span className="count">{archived.length}</span></div><nav className="trip-list archive-list" aria-label="Arkiverade resor">{archived.map(tripLink)}</nav></div>}
