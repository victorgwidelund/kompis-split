...
import type { AdminResponse, Category, DashboardResponse, Expense, InvitationPreview, InvitationResult, QuickTab, QuickTabSummary, SessionResponse, StatisticsResponse, Trip, User, View } from "./types/models";
...
export default function App() {
  ...
  useEffect(() => { void (async () => {
    try {
      ...
      const session = await api<SessionResponse>("/api/session"); setVersion(session.version || "dev"); setNeedsSetup(session.needsSetup);
      if (session.authenticated && session.user) {
        ...
        else setAuthMode(session.needsSetup ? "setup" : preview?.kind === "quick_tab" ? "quick-guest" : preview ? "register" : "login");
      }
  ...
  return <><Shell user={shellUser} version={version} trips={dashboard?.trips || []} view={view} guestMode={guestMode} onNavigate={(next) => void navigate(next)} onNewTrip={() => setGlobalDialog("trip")} onNewQuickTab={() => setGlobalDialog("quick")} onLogout={() => void api("/api/logout", { method: "POST", body: {} }).then(() => location.reload())}>
    {view.page === "dashboard" && user && dashboard && <DashboardPage user={user} dashboard={dashboard} quickTabs={quickTabs} categories={categories} onNavigate={(next) => void navigate(next)} onNewTrip={() => setGlobalDialog("trip")} onNewQuickTab={() => setGlobalDialog("quick")} onInviteFriend={() => setGlobalDialog("friend")} />}
    ...
    {view.page === "admin" && admin && user && <AdminPage data={admin} currentUserId={user.id} onBack={() => void navigate({ page: "dashboard" })} onRefresh={() => void reloadAdmin()} onOpenTrip={(id) => void navigate({ page: "trip", id })} onUserUpdate={(id, update) => void api(`/api/admin/users/${id}`, { method: "PATCH", body: update }).then(reloadAdmin).then(() => notify("Kontot uppdaterades"))} onTripArchive={(id, archived) => void api(`/api/trips/${id}/archive`, { method: "POST", body: { archived } }).then(() => Promise.all([reloadAdmin(), refreshDashboard()])).then(() => notify("Resan uppdaterades"))} onTripRestore={(id) => void api(`/api/trips/${id}/trash`, { method: "POST", body: { deleted: false } }).then(() => Promise.all([reloadAdmin(), refreshDashboard()])).then(() => notify("Resan återställdes"))} />}
  </Shell>
  <NewTripDialog open={globalDialog === "trip"} onClose={() => setGlobalDialog(null)} onCreated={async (created) => { await refreshDashboard(); setTrip(created); setView({ page: "trip", id: created.id }); history.replaceState(null, "", `${location.pathname}#trip-${created.id}`); setTripDialog("invite"); notify("Resan skapades — bjud nu in gänget"); }} />
